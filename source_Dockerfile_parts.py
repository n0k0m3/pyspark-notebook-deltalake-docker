...
def modify_mamba(mamba_part: str) -> str:
    mamba_part = "\n"\
        .join([x for x in mamba_part if "fix-permissions" not in x and "NB_USER" not in x])\
...
def checkout_base_notebook():
    ...
    base_notebook_dockerfile_part += re.findall(
        r"USER root([\w\W]+)\# Configure environment",
        base_notebook_dockerfile)[0].strip() + "\n"
...
def checkout_minimal_notebook():
    ...
    minimal_notebook_dockerfile_part += re.findall(
        r"USER root([\w\W]+)\# Switch back to jovyan",
        minimal_notebook_dockerfile)[0].strip() + "\n"
...
def checkout_scipy_notebook():
    ...
    apt_part = re.findall(
        ...
        scipy_notebook_dockerfile)[0].strip() + "\n"

    mamba_part = re.findall(r"USER \${NB_UID}([\w\W]+)# Import matplotlib",
                            scipy_notebook_dockerfile)[0].strip().split("\n")
    mamba_part = modify_mamba(mamba_part)\
        .replace("rm -rf /tmp/facets && \\", "rm -rf /tmp/facets")
...
def checkout_pyspark_notebook():
    ...
    fix_part = re.findall(r"(\# Fix DL4006[\w\W]+)USER root",
                          pyspark_notebook_dockerfile)[0].strip() + "\n"
    spark_part = re.findall(r"(\# Spark dependencies[\w\W]+)USER \${NB_UID}", pyspark_notebook_dockerfile)[0]\
                    .replace("RUN fix-permissions \"/etc/ipython/\"","").strip() + "\n"
    mamba_part = re.findall(r"USER \${NB_UID}([\w\W]+)WORKDIR \"\${HOME}\"",
                            pyspark_notebook_dockerfile)[0].strip().split("\n")

    mamba_part = modify_mamba(mamba_part)

    pyspark_notebook_dockerfile_part += fix_part + "\n" + spark_part + "\n" + mamba_part + "\n"
...
def checkout_all_spark_notebook():
    all_spark_notebook_dockerfile_part = header(
    ...
    mamba_part = re.findall(
        ...
        all_spark_notebook_dockerfile)[0].strip().split("\n")
    mamba_part = modify_mamba(mamba_part)\
                    .replace("install --sys-prefix && \\", "install --sys-prefix")
...
def checkout_r_notebook():
    ...
    apt_part = re.findall(r"USER root([\w\W]+)USER \${NB_UID}",
                          r_notebook_dockerfile)[0].strip() + "\n"
    mamba_part = re.findall(r"USER \${NB_UID}([\w\W]+)",
                            r_notebook_dockerfile)[0].strip().split("\n")
